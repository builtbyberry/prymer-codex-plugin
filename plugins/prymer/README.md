...
On first use, the signed Prymer helper opens Prymer in your browser to authorize. No host OAuth command or token copy is required.

Codex will ask you to trust the plugin hooks on first run (`/hooks`).

## After install

- **The MCP connection** registers as `prymer` when the plugin loads; its local broker carries no Cloud credential.
